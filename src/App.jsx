...
function bestLiftRatio(sessions, measurements, nameRegex) {
  const latestWeightEntry = (measurements || []).slice().sort((a, b) => a.date.localeCompare(b.date)).pop();
...
function bestRepsFor(sessions, nameRegex) {
  let max = 0;
...
function totalPoolMinutes(sessions) {
  let total = 0;
  (sessions || []).forEach((s) => (s.cardio || []).forEach((c) => {
...
function currentMaxStreak(sessions) {
  const uniqueDates = [...new Set((sessions || []).map((s) => s.date))].sort();
  const activeDates = new Set(uniqueDates);
  let best = 0;
  uniqueDates.forEach((date) => {
...
function maxWeeklyVolume(sessions) {
  const byWeek = {};
  (sessions || []).forEach((s) => {
...
function maxTreadmillDistance(sessions) {
  let max = 0;
  (sessions || []).forEach((s) => {
    (s.cardio || []).forEach((c) => {
